...
## Separating water quality data collected at different depths 
...
hours.  A pressure transducer is used to calculate the depth of each 
measurement.  Here, we can bin these into 1 meter increments.

```{r bin-depths}
# Subset water quality data by depth into 1 m bins
waq_0m<-subset(waq_instantaneous,sensorDepth<1)
waq_1m<-subset(waq_instantaneous,sensorDepth>=1 & sensorDepth<2)
waq_2m<-subset(waq_instantaneous,sensorDepth>=2 & sensorDepth<3)
waq_3m<-subset(waq_instantaneous,sensorDepth>=3 & sensorDepth<4)
waq_4m<-subset(waq_instantaneous,sensorDepth>=4 & sensorDepth<5)
waq_5m<-subset(waq_instantaneous,sensorDepth>=5 & sensorDepth<6)
waq_6m<-subset(waq_instantaneous,sensorDepth==6)
```

## Plot dissolved oxygen data

Now that we have our data binned by depth, we can plot it as a time series. 
